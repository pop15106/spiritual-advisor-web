...
export default function AstrologySection() {
    ...
    const calculateChart = async () => {
        ...
        await astrologyApi.calculateStream(
            birthDate,
            birthTime,
            selectedCity.name,
            (data: any) => {
                setAstroData({ ...data, interpretation: '' } as ExtendedAstroResponse);
            },
            (chunk: string) => {
                setAstroData(prev => {
                    if (!prev) return prev;
                ...
                <>
                                    ...
                                    {astroData.planetDetails && Object.entries(astroData.planetDetails).map(([name, detail]) => (
                                        <tr key={name} className="border-b border-slate-100 hover:bg-slate-50">
                                            ...
                                            <td className="p-1.5 text-center">
                                                <span className={`px-1.5 py-0.5 rounded text-[9px] ${detail.element === '火' ? 'bg-red-100 text-red-700' :
                                    ...
                                    return (
                                        <div key={e} className={`flex items-center gap-2 p-1.5 rounded ${isDominant ? `bg-${colors[e as keyof typeof colors]}-50 ring-1 ring-${colors[e as keyof typeof colors]}-300` : ''}`}>
                                            ...
                                            <div className="w-16 h-2 bg-slate-200 rounded-full overflow-hidden">
                                                <div className={`h-full rounded-full ${e === "火" ? "bg-red-500" : e === "土" ? "bg-green-500" : e === "風" ? "bg-yellow-500" : "bg-blue-500"
                        ...
                        <div className="bg-white rounded-lg border border-indigo-100 p-3 mb-4">
                            ...
                            <div className="grid grid-cols-2 gap-1.5">
                                {astroData.aspects.slice(0, 16).map((asp, idx) => (
                                    <div key={idx} className={`flex items-center gap-1.5 px-2 py-1 rounded text-[10px] ${asp.nature === '吉' ? 'bg-green-50 text-green-800' :
                        ...
                        <div className="bg-white rounded-lg border border-indigo-100 p-3 mb-4">
                            ...
                            <div className="grid grid-cols-4 gap-1.5">
                                {astroData.houses.map((h, idx) => (
                                    <div key={idx} className={`rounded-lg p-1.5 text-center ${[0, 3, 6, 9].includes(idx) ? 'bg-indigo-100' : 'bg-slate-50'
                                        }`}>
                                        <div className={`text-[10px] font-bold ${[0, 3, 6, 9].includes(idx) ? 'text-indigo-700' : 'text-slate-600'
                                            }`}>{h.house}宮</div>
                    ...
                    <div className="bg-white rounded-lg border border-indigo-100 p-3 mb-4">
                        ...
                        <div className="text-xs text-slate-700 space-y-2 leading-relaxed">
                            {(!astroData.interpretation || astroData.interpretation.length === 0) && (
                                <div className="flex items-center gap-3 text-indigo-600 animate-pulse py-4">
                                    <svg className="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24">
                                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                    <span>占星大師正在詳批中，請稍候...</span>
                                </div>
                            )}
                    ...
                    <button onClick={() => setAstroData(null)} disabled={loading} className={`w-full py-2 rounded-lg text-sm ${loading ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                        {loading ? '分析中...' : '重新計算'}
                    </button>
                </>
            )}
